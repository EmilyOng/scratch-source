...
/***/ function(module, exports) {
	...
	*/
	...
	Blockly.Workspace.prototype.deleteVariable=function(a){for(var b=this.getVariableUses(a),c=0,d;d=b[c];c++)if(d.type==Blockly.PROCEDURES_DEFINITION_BLOCK_TYPE||"procedures_defreturn"==d.type){b=d.getFieldValue("NAME");Blockly.alert(Blockly.Msg.CANNOT_DELETE_VARIABLE_PROCEDURE.replace("%1",a).replace("%2",b));return}var e=this,f=e.getVariable(a);1<b.length?Blockly.confirm(Blockly.Msg.DELETE_VARIABLE_CONFIRMATION.replace("%1",b.length).replace("%2",a),function(a){a&&e.deleteVariableInternal_(f)}):this.deleteVariableInternal_(f)};
	...
	Blockly.Variables.createVariable=function(a,b,c){var d="";d=c===Blockly.LIST_VARIABLE_TYPE?Blockly.Msg.NEW_LIST_TITLE:c===Blockly.BROADCAST_MESSAGE_VARIABLE_TYPE?Blockly.Msg.NEW_BROADCAST_MESSAGE_TITLE:Blockly.Msg.NEW_VARIABLE_TITLE;var e=function(f){Blockly.Variables.promptName(d,f,function(d){if(d)if(a.getVariable(d))Blockly.alert(Blockly.Msg.VARIABLE_ALREADY_EXISTS.replace("%1",d.toLowerCase()),function(){e(d)});else if(Blockly.Procedures.isNameUsed(d,a)){var f=a.createVariable(d,c),g=a.getFlyout();
	f=f.getId();g.setCheckboxState&&g.setCheckboxState(f,!0);b&&b(d)}else Blockly.alert(Blockly.Msg.PROCEDURE_ALREADY_EXISTS.replace("%1",d.toLowerCase()),function(){e(d)});else b&&b(null)})};e("")};
	...
	Blockly.Xml.blockToDom=function(a,b){var c=goog.dom.createDom(a.isShadow()?"shadow":"block");c.setAttribute("type",a.type);b||c.setAttribute("id",a.id);if(a.mutationToDom){var d=a.mutationToDom();d&&(d.hasChildNodes()||d.hasAttributes())&&c.appendChild(d)}d=0;for(var e;e=a.inputList[d];d++)for(var f=0,g;g=e.fieldRow[f];f++){var h=g;if(h.name&&h.SERIALIZABLE){g=goog.dom.createDom("field",null,h.getValue());g.setAttribute("name",h.name);if(h instanceof Blockly.FieldVariable||h instanceof Blockly.FieldVariableGetter){var k=
	a.workspace.getVariable(h.getValue());k?(g.setAttribute("id",k.getId()),g.setAttribute("variabletype",k.type)):h instanceof Blockly.FieldVariable&&(h=h.getVariableTypes_(),1==h.length&&g.setAttribute("variabletype",h[0]))}c.appendChild(g)}}if(d=a.getCommentText())d=goog.dom.createDom("comment",null,d),"object"==typeof a.comment&&(d.setAttribute("pinned",a.comment.isVisible()),e=a.comment.getBubbleSize(),d.setAttribute("h",e.height),d.setAttribute("w",e.width)),c.appendChild(d);a.data&&(d=goog.dom.createDom("data",
	null,a.data),c.appendChild(d));for(d=0;e=a.inputList[d];d++){var l;g=!0;e.type!=Blockly.DUMMY_INPUT&&(h=e.connection.targetBlock(),e.type==Blockly.INPUT_VALUE?l=goog.dom.createDom("value"):e.type==Blockly.NEXT_STATEMENT&&(l=goog.dom.createDom("statement")),f=e.connection.getShadowDom(),!f||h&&h.isShadow()||l.appendChild(Blockly.Xml.cloneShadow_(f)),h&&(l.appendChild(Blockly.Xml.blockToDom(h,b)),g=!1),l.setAttribute("name",e.name),g||c.appendChild(l))}a.inputsInlineDefault!=a.inputsInline&&c.setAttribute("inline",
	a.inputsInline);a.isCollapsed()&&c.setAttribute("collapsed",!0);a.disabled&&c.setAttribute("disabled",!0);a.isDeletable()||a.isShadow()||c.setAttribute("deletable",!1);a.isMovable()||a.isShadow()||c.setAttribute("movable",!1);a.isEditable()||c.setAttribute("editable",!1);if(d=a.getNextBlock())l=goog.dom.createDom("next",null,Blockly.Xml.blockToDom(d,b)),c.appendChild(l);f=a.nextConnection&&a.nextConnection.getShadowDom();!f||d&&d.isShadow()||l.appendChild(Blockly.Xml.cloneShadow_(f));return c};
	...
	Blockly.FieldVariable.prototype.initModel=function(){this.getValue()||this.setValue(Blockly.Variables.generateUniqueName(this.sourceBlock_.isInFlyout?this.sourceBlock_.workspace.targetWorkspace:this.sourceBlock_.workspace));this.sourceBlock_.isInFlyout||(1==this.getVariableTypes_().length?this.sourceBlock_.workspace.createVariable(this.getValue(),this.getVariableTypes_()[0]):this.sourceBlock_.workspace.createVariable(this.getValue()))};
	...
	Blockly.FieldVariable.dropdownCreate=function(){var a=[],b=this.getText(),c=b?!0:!1,d=null;this.sourceBlock_&&(d=this.sourceBlock_.workspace);var e=!1;if(d){var f=this.getVariableTypes_();a=[];for(var g=0;g<f.length;g++){var h=f[g];h==Blockly.BROADCAST_MESSAGE_VARIABLE_TYPE&&(e=!0);h=d.getVariablesOfType(h);a=a.concat(h)}for(g=0;g<a.length;g++)if(c&&goog.string.caseInsensitiveEquals(a[g].name,b)){c=!1;break}}c&&d&&!e&&(g=d.createVariable(b),a.push(g));a.sort(Blockly.VariableModel.compareByName);c=
	[];for(g=0;g<a.length;g++)c[g]=[a[g].name,a[g].getId()];e?d.isFlyout||c.push([Blockly.Msg.NEW_BROADCAST_MESSAGE,Blockly.NEW_BROADCAST_MESSAGE_ID]):(c.push([Blockly.Msg.RENAME_VARIABLE,Blockly.RENAME_VARIABLE_ID]),Blockly.Msg.DELETE_VARIABLE&&c.push([Blockly.Msg.DELETE_VARIABLE.replace("%1",b),Blockly.DELETE_VARIABLE_ID]));return c};
	Blockly.FieldVariable.prototype.onItemSelected=function(a,b){var c=b.getValue();if(this.sourceBlock_&&this.sourceBlock_.workspace){var d=this.sourceBlock_.workspace,e=d.getVariableById(c);if(e)var f=e.name;else{if(c==Blockly.RENAME_VARIABLE_ID){c=this.getText();e=d.getVariable(c);Blockly.Variables.renameVariable(d,e);return}if(c==Blockly.DELETE_VARIABLE_ID){d.deleteVariable(this.getText());return}if(c==Blockly.NEW_BROADCAST_MESSAGE_ID){var g=this;Blockly.Variables.createVariable(d,function(a){g.setValue(a)},
	Blockly.BROADCAST_MESSAGE_VARIABLE_TYPE);return}}f=this.callValidator(f)}null!==f&&this.setValue(f)};Blockly.Generator=function(a){this.name_=a;this.FUNCTION_NAME_PLACEHOLDER_REGEXP_=new RegExp(this.FUNCTION_NAME_PLACEHOLDER_,"g")};Blockly.Generator.NAME_TYPE="generated_function";Blockly.Generator.prototype.INFINITE_LOOP_TRAP=null;Blockly.Generator.prototype.STATEMENT_PREFIX=null;Blockly.Generator.prototype.INDENT="  ";Blockly.Generator.prototype.COMMENT_WRAP=60;Blockly.Generator.prototype.ORDER_OVERRIDES=[];
	...
	Blockly.Procedures.editProcedureCallback_=function(a){if(a.type==Blockly.PROCEDURES_DEFINITION_BLOCK_TYPE){a=a.getInput("custom_block");if(!a){alert("Bad input");return}a=a.connection;if(!a){alert("Bad connection");return}a=a.targetBlock();if(!a||!a.type==Blockly.PROCEDURES_PROTOTYPE_BLOCK_TYPE){alert("Bad inner block");return}}else a.type==Blockly.PROCEDURES_CALL_BLOCK_TYPE&&(a=Blockly.Procedures.getPrototypeBlock(a.getProcCode(),a.workspace));Blockly.Procedures.externalProcedureDefCallback(a.mutationToDom(),
	Blockly.Procedures.editProcedureCallbackFactory_(a))};Blockly.Procedures.editProcedureCallbackFactory_=function(a){return function(b){b&&Blockly.Procedures.mutateCallersAndPrototype(a.getProcCode(),a.workspace,b)}};Blockly.Procedures.externalProcedureDefCallback=function(){alert("External procedure editor must be override Blockly.Procedures.externalProcedureDefCallback")};Blockly.Procedures.makeEditOption=function(a){return{enabled:!0,text:Blockly.Msg.EDIT_PROCEDURE,callback:function(){Blockly.Procedures.editProcedureCallback_(a)}}};
	...
	Blockly.Flyout.prototype.getAddedVariables_=function(a){var b=this.targetWorkspace_.getAllVariables(),c=[];if(a.length!=b.length)for(var d=0;d<b.length;d++){var e=b[d];a.includes(e)||c.push(e)}return c};
	Blockly.Flyout.prototype.createBlock=function(a){var b=null;Blockly.Events.disable();var c=this.targetWorkspace_.getAllVariables();this.targetWorkspace_.setResizesEnabled(!1);try{b=this.placeNewBlock_(a),Blockly.hideChaff()}finally{Blockly.Events.enable()}a=this.getAddedVariables_(c);if(Blockly.Events.isEnabled())for(Blockly.Events.setGroup(!0),Blockly.Events.fire(new Blockly.Events.Create(b)),c=0;c<a.length;c++)Blockly.Events.fire(new Blockly.Events.VarCreate(a[c]));this.autoClose&&this.hide();return b};
